...
jest.mock("@pui/icons/lib/icons/IconCorrectFilled", () => () => <div>MockIconCorrectFilled</div>);
jest.mock("@pui/icons/lib/icons/IconErrorFilled", () => () => <div>MockIconErrorFilled</div>);
  ...
  describe('Test Progress Component', () => {
    describe('test classname is passed correctly', () => {
      it('should render progress component with -success classname when percent is 100 regardless of status', () => {
        ...
        expect(container.getElementsByClassName('pui-progress pui-progress-status-success')[0]).toBeInTheDocument()
        expect(container.getElementsByClassName('pui-progress-text pui-progress-text-status-success')[0]).toBeInTheDocument()
      });

      it('should render progress component with classname depending on the status', () => {
        const {container: normalContainer} = render(<Progress percent={50} status='error'/>)
        expect(normalContainer.getElementsByClassName('pui-progress pui-progress-status-error')[0]).toBeInTheDocument()
        expect(normalContainer.getElementsByClassName('pui-progress-text pui-progress-text-status-error')[0]).toBeInTheDocument()
      });
    });

    it('should render an inner progress bar with passed percent', () => {
      const {container} = render(<Progress percent={70} />)
      expect(container.getElementsByClassName('pui-progress-inner')[0]).toHaveStyle({width: '70%'})
    });

    it('should not show info when showInfo is set to false', () => {
      render(<Progress percent={25} status='success' showInfo={false} />)
      expect(screen.queryByText('MockIconCorrectFilled')).not.toBeInTheDocument()
    });

    it('should show close button when status is normal and call onStop when click', () => {
      const mockFn = jest.fn()
      const { container } = render(<Progress percent={25} status='normal' onStop={mockFn}/>)
