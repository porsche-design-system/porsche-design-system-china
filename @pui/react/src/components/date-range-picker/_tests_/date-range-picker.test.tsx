...
describe('date-range-picker', () => {
  ...
  it('should be unavailable dates out of range', () => {
    ...
    const unavailableElement = document.querySelector('.pui-date-range-picker-calendar-unavailable') as HTMLElement;
  ...
  it('should select automatically when set initial date', async () => {
    ...
    await userEvent.click(dateInit);
  ...
  it('should start date available with range time without selecting initial time', async () => {
    ...
    render(<DateRangePicker range={value} placeholderStartDate='开始日期' placeholderEndDate='结束日期' />);
    ...
    expect(startDateClassName).toBe('pui-date-range-picker-calendar-block        ');
  });

  it('should show calendar today when no set value and range', async () => {
    render(<DateRangePicker placeholderStartDate='开始日期' placeholderEndDate='结束日期' />);

    const startButton = screen.getByText('开始日期');

    await userEvent.click(startButton);

    const calendarTodayElement = document.querySelector('.pui-date-range-picker-calendar-today') as HTMLElement;
  ...
  describe('click delete button', () => {
    it('should delete date initial when click start delete button', async () => {
      render(<DateRangePicker value={['2023-03-27', '2023-03-28']} showClearButton placeholderStartDate='开始日期' />);
    ...
    it('should delete date end when click end delete button', async () => {
      render(<DateRangePicker value={['2023-03-27', '2023-03-28']} showClearButton placeholderEndDate='结束日期' />);
  ...
  describe('mustPickStartEnd property', () => {
    it('should clean all picked date when click start delete button', async () => {
      ...
      render(<DateRangePicker mustPickStartEnd value={['2023-03-27', '2023-03-28']} showClearButton
                              placeholderStartDate='开始日期' placeholderEndDate='结束日期' />);
  ...
  describe('range type is string', () => {
    it('should set range when range property is string', () => {
      ...
      expect(unavailableElement).toBeTruthy();
      expect(outOfRangeDate).toHaveClass('pui-date-range-picker-calendar-unavailable')
    });
  ...
  describe('picker box active', () => {
    ...
    it('should active end pick box when select start date', async () => {
      ...
      const pickerBox = document.querySelectorAll('.pui-date-range-picker-box')[1];
    ...
    it('should active start pick box when select end date', async () => {
      ...
      const pickerBox = document.querySelectorAll('.pui-date-range-picker-box')[0];
