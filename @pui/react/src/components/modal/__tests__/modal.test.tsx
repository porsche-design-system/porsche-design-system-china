...
describe('test modal component', () => {
  describe('test modal', () => {
    it('should render correct when props are true', () => {
      ...
      // divider test
      expect(footerOkBtn.parentElement?.parentElement).toHaveClass('pui-modal-footer-divider-true')
      ...
      // show title icon
      expect(screen.getByLabelText('icon_-informationFilled')).toBeInTheDocument()
    ...
    it('should render correct when props are false', () => {
      ...
      // show title icon
      expect(screen.queryByLabelText('icon_-informationFilled')).not.toBeInTheDocument()
    });
  ...
  describe('test modal.alert', () => {
    ...
    it('should call remove child twice when loading promise return value is undefined', () => {
      const okFn = jest.fn()
      Modal.alert('Alert Modal title', <div>modal</div>, okFn, 'Alert好的Promise')
      const mockFn = jest.fn();
      jest.spyOn(document.body, 'removeChild').mockImplementation(mockFn)
    ...
    it('should call remove child twice when loading promise return value is not undefined', async () => {
      ...
      const mockFn = jest.fn();
      jest.spyOn(document.body, 'removeChild').mockImplementation(mockFn)
      Modal.alert('Alert Modal title', <div>modal</div>, okFn, 'Alert好的Promise02')
  ...
  describe('test modal.confirm', () => {
    it('should render confirm component correct when call confirm function',  async () => {
      ...
      await Modal.confirm('ConfirmModal', <div>confirm content</div>, okFn, cancelFn, '确定', '关闭')
      Object.defineProperty(global.document.body, 'removeChild', { value: removeChildFn });
    ...
    it('should call remove child twice when ok function is undefined', () => {
      Modal.confirm('Confirm Modal title', <div>modal</div>, undefined, undefined, '确认按钮')
      const mockFn = jest.fn();
      jest.spyOn(document.body, 'removeChild').mockImplementation(mockFn)
    ...
    it('should call remove child twice when loading promise return value is undefined', () => {
      const okFn = jest.fn()
      Modal.confirm('Confirm Modal title', <div>modal</div>, okFn, undefined, '提交 Confirm Promise')
      const mockFn = jest.fn();
      jest.spyOn(document.body, 'removeChild').mockImplementation(mockFn)
    ...
    it('should call remove child twice when loading promise return value is not undefined', async () => {
      ...
      const mockFn = jest.fn();
      jest.spyOn(document.body, 'removeChild').mockImplementation(mockFn)
      Modal.confirm('Confirm Modal title', <div>modal</div>, okFn, undefined, '提交 Confirm Promise02')
