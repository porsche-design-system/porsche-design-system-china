...
jet.extend(jeDatePick.prototype, {
  ...
  renderDate: function (x) {
    ...
    if (jet.isObj(opts.theme)) {
      var styleDiv = document.createElement('style'),
        stCell = '.pui-pick-date' + searandom(),
        t = opts.theme,
    ...
    jet.extend(that, {
      ...
      hmsClick: function (idx, num) {
        var pidx = parseInt(num),
          ...
          paridx = parseInt(idx),
          act = 'action',
          mhms = that.$opts.showSecend ? ['hh', 'mm', 'ss'] : ['hh', 'mm'],
          ulCell = $Q('.pui-pick-date-time', that.dateCell).querySelectorAll(
            'ul'
          )[pidx],
      ...
      timeBtn: function () {
        var timeCell = $Q('.pui-pick-date-time', elCell),
      ...
      sureBtn: function () {
        ...
        if (range) {
          var dateLen = opts.allowNullDate ? 0 : 1
  ...
  eachTime: function (hmsArr, type) {
    ...
    var stepArr = that.$opts.showSecend ? [24, 60, 60] : [24, 60]
    jet.each(stepArr, function (s, lens) {
      timeArr[s] = []
      var unhmsVal =
  ...
  tips: function (text, time) {
    var that = this,
      tipCls = $Q('.pui-pick-date-tips', that.dateCell),
      tipTime
  ...
  locateScroll: function () {
    var that = this,
      ulCell = $Q('.pui-pick-date-time', that.dateCell).querySelectorAll('ul')
